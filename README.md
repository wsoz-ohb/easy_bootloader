...
## 升级流程示意
1. APP 运行中 -> 上位机发送“触发升级”命令（携带新版本/日期）。
2. APP 对比版本号，不一致则写 Flag=1 + 新版本信息后复位。
3. Bootloader 检测 Flag=1，留在刷写模式并擦除 APP 区。
4. 上位机按包发送固件，每包等待 ACK；Bootloader 写入 Flash。
5. 最后一包完成，Bootloader 写 Flag=2 并跳回 APP，新版本开始运行。
...
![image-20251209005307545](README.assets/image-20251209005307545.png)



## 待优化问题

目前框架还有几个需要优化的地方，最重要的是一个**逻辑问题**主要是触发升级之后就会立刻对我们的标志位区的版本号和信息进行修改之后重启到bootloader区进行等待刷写。后续我准备将这个信息打包到最后一帧数据包中或则再完成数据包之后再发一帧完成包来完善当前的逻辑。
